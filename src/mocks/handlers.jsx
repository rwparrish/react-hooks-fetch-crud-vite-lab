...
export const handlers = [
  http.get("http://localhost:4000/questions", (req, res, ctx) => {
    return res(ctx.json(questions));
  }),
  http.post("http://localhost:4000/questions", (req, res, ctx) => {
    const id = questions[questions.length - 1]?.id + 1 || 1;
    const question = { id, ...req.body };
    questions.push(question);
    return res(ctx.json(question));
  }),
  http.delete("http://localhost:4000/questions/:id", (req, res, ctx) => {
    const { id } = req.params;
  ...
  http.patch("http://localhost:4000/questions/:id", (req, res, ctx) => {
    const { id } = req.params;
